...
cache_bayer = TTLCache(maxsize=100, ttl=86400)
...
def get_ideal_treatment(start_value, end_value):
	...
	Rounding values to one decimal to optimize the cache
    ...
    end_value_delta = round(end_value_delta)
    ...
    return [round(start_value)] + [round(x + start_value) for x in get_ideal_treatment_origin(end_value_delta)]

@cached(cache)
def get_ideal_treatment_origin(end_value_delta):
    """"
	Returns values for the ideal treatment trajectory, located at the origin
	"""
    glucose_values = []
    ...
    for i in range(n):
        ...
        if end_value_delta == 0:
            ...
            new_value = get_glucose_from_carbs(0, end_value_delta, t)
            glucose_values.append(new_value)
        else:
            new_value = (1 - percent_effect_remaining(t - 10.0, 360.0, 75.0)) * end_value_delta
            glucose_values.append(new_value)
...
def get_glucose_penalties_for_pairs(true_values, derived_values, target=105, penalty_type='bayer'):
    ...
    for i in range(0, len(true_values)):
        penalty = get_glucose_penalty_for_pair(true_values[i], derived_values[i], target=target,
...
def get_glucose_penalty_for_pair(true_blood_glucose, derived_blood_glucose, target=105, penalty_type='bayer'):
    ...
    if penalty_type == 'kovatchev':
        ...
        return derived_blood_glucose - true_blood_glucose
    else:
