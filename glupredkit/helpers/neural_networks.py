...
def prepare_sequences(data, labels, window_size, real_time, step_size=1):
    ...
    # TODO: Merge "columns" into the same list
    ...
    for i in range(0, len(data) - window_size, step_size):
        ...
        # TODO: treat each feature differently
        #sequence = []
        #for col in sequence_columns:
        #    sequence.extend(data[col][i:i + window_size].tolist())
        # sequence = [data[col][i:i + window_size] for col in sequence_columns]

        #print("SEQUENCE: ", np.array(sequence))
...
def create_dataframe(sequences, targets, dates):
    ...
    #sequences_as_strings = [','.join(['[' + ','.join(map(str, subseq)) + ']' for subseq in seq]) for seq in sequences]
    sequences_as_strings = [str(list(map(list, seq))) for seq in sequences]

    print(sequences_as_strings)
