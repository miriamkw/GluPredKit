...
class Preprocessor(BasePreprocessor):
    ...
    def preprocess(self, df):
        ...
        # Adding imputed flag to NaN CGM values for test data
        test_df['imputed'] = test_df['CGM'].isna()

        # Add target for test data before interpolation to perceive NaN values
        test_df = self.add_target(test_df)

        # TODO: CHANGE BACK INTERPOLATION METHOD! Limit direction can stay anyway.
    ...
    def add_target(self, df):
        ...
        if 'imputed' in df.columns:
            ...
            df.loc[:, 'imputed'] = df['imputed'].shift(-target_index)
