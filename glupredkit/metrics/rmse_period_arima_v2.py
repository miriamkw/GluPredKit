...
class Metric(BaseMetric):
    ...
    def __call__(self, y_true, y_pred, test_data):
        ...
        for log in activity_logs:
            ...
            history_start_time = start_time - pd.Timedelta(minutes=(history_steps + self.PH) * 5) 
            history_end_time = history_start_time + pd.Timedelta(minutes=(history_steps) * 5)
            prev_start_time = start_time - pd.Timedelta(minutes=(self.PH) * 5)

            history_indices = (y_true.index >= history_start_time) & (y_true.index <= history_end_time)
            previous_indices = (y_true.index >= prev_start_time)
            indices = (y_true.index >= start_time) & (y_true.index <= end_time)

            # ARIMA forecasts during the activity period
            # version2 only uses the 24 hours previous data + newly observed data during PA
            forecasts = []
            history = np.array(y_true)[history_indices]
