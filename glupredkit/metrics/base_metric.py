...
class BaseMetric(ABC):
    ...
    def __call__(self, y_true: List[float], y_pred: List[float]) -> any:
        ...
        if any(np.isnan(y) for y in y_pred):
            warnings.warn("NaN values detected in metric input predicted values", UserWarning)
        if any(np.isnan(y) for y in y_true):
            warnings.warn("NaN values detected in metric input true values", UserWarning)
